...
namespace BingRewards
{
	public class BingSearch
	{
		...
		private void DoProcessing(bool slow, int startWithUser)
		{
			...
			do
			{
				if (startWithUser <= _currentUserIndex)
				{
					...
					{
						Logger.Instance.Log(exception);
						//if we're failing, let's leave the browser open so we can look at it...
						//CloseBrowser(_browserDriverCurrentSearchUser);
					}
				}
			} while (GetNextUser());
		...
		private static void NavigateToUrl(IWebDriver browserDriver, string url, int minWait, bool loadTwice = false)
		{
            ...
            //this will try endlessly when things go wrong, added limit
			//this is rarely used, but occasionally the Navigate doesn't make it to the page.
			while (string.Compare(browserDriver.Url, url, StringComparison.CurrentCultureIgnoreCase) != 0 && loopCount < loopLimit)
			{
				browserDriver.Navigate().GoToUrl(url);
				Thread.Sleep(minWait);
			    loopCount++;
		...
		private void LogIn()
		{
			...
			IWebElement login = _browserDriverCurrentSearchUser.FindElement(By.Name("loginfmt"));//config
			IWebElement next = _browserDriverCurrentSearchUser.FindElement(By.Id("idSIButton9"));
            ...
            IWebElement submit = _browserDriverCurrentSearchUser.FindElement(By.Id("idSIButton9"));
            passwd.SendKeys(_currentUser.Password);
		    submit.Click();

            //https://account.microsoft.com/?lang=en-US
            if (string.Compare(_browserDriverCurrentSearchUser.Url, "https://account.microsoft.com/?lang=en-US",
					StringComparison.CurrentCultureIgnoreCase) != 0)
			{
				//try navigating to the summary page:
				NavigateToUrl(_browserDriverCurrentSearchUser, "https://account.live.com/summarypage.aspx", _minWait);
				//if we still werent able to get to that page then!!!
				if (string.Compare(_browserDriverCurrentSearchUser.Url, "https://account.live.com/summarypage.aspx",
				   StringComparison.CurrentCultureIgnoreCase) != 0)
				{
					Logger.Instance.Log("Error logging in!");
