...
B_OPTIONS_PAGE_CREATE( b_options_font_page )
{
	...
	for ( a = 0; ; a++ )
	{
		...
		c_new_label( cat, lang_phrase_quick( items[a].name ), 10, y, sx, -1, 0 );
		...
		items[a].input = c_new_text( cat, items[a].display, sx+10, y, pw-20-sx-110, -1, 0 );
		items[a].button = c_new_pushbutton( cat, lang_phrase_quick( "choose" ), pw-110, y, 100, -1, 0 );
		
		c_widget_disable( items[a].input );
		
		c_new_event_handler( items[a].button, C_EVENT_BUTTON_PRESS, b_opt_font_choose_click );
