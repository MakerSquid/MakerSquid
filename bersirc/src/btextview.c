...
/*
void c_btv_handle_draw( CEvent *e, void *data )
{
	c_canvas_paint_clear( e->caller, 0xFF00FF00 );
	
	c_canvas_paint_text( e->caller, 10, 10, 0xFFFF0000, "Hello, World! :|" );
}
*/
...
int cache_font_get_string_width( BTextView *w, char *text, int len ) {
	...
	strncpy( txt, text, len );
	txt[len] = 0;
	
	for ( a = 0; a < fw_cache_size; a++ )
	{
		if ( fw_cache_list[a].w == w->w && !strcmp( text, fw_cache_list[a].text ) )
		{
			printf( "MATCHED cached font width.\n" );
			if ( fw_cache_list[a].width <= 0 )
				fw_cache_list[a].width = 0;//c_font_get_string_width( w->w, text, len );
			
			//DBG_BTV_PRINTF( "Retrieved font width from cache! %d\n", fw_cache_list[a].width);
			
			free( txt );
			
			return fw_cache_list[a].width;
		}
	}
	
	fw_cache_size++;
	
	fw_cache_list = (FontWidthCache *)realloc( fw_cache_list, fw_cache_size * sizeof(FontWidthCache) );
	printf( "Reallocating list pointer to %d bytes!\n", fw_cache_size * sizeof(FontWidthCache) );
	fw_cache_list[a].w = w->w;
	fw_cache_list[a].text = (char *)malloc( len + 1 );
	strcpy( fw_cache_list[a].text, txt );
	fw_cache_list[a].width = 0;//c_font_get_string_width( w->w, text, len );
...
void c_btv_handle_mouse_left_down( CEvent *e, void *data )
{
	int *dat = (int *)data;
	int x = dat[0];
	int y = dat[1];
...
void c_btv_handle_mouse_right_release( CEvent *e, void *data )
{
	int *dat = (int *)data;
	int x = dat[0];
	int y = dat[1];
...
void c_btv_handle_mouse_left_release( CEvent *e, void *data )
{
	BTextView *btv;
	btv_linkarea *curr;
	BTVLine *line;
...
void c_btv_handle_mouse_move( CEvent *e, void *data )
{
	int *dat = (int *)data;
	int x = dat[0];
	int y = dat[1];
...
void c_btv_handle_resize( CEvent *e, void *data )
{
	//DBG_BTV_PRINTF( "c_btv_handle_resize( )\n" );
}

void c_btv_handle_destroy( CEvent *e, void *data )
{
	BTextView *bv = btv_by_widget( e->caller );
